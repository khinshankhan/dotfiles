...
- [[#tricks-n-gimmicks-for-sweeter-configuration-ii][Tricks n Gimmicks for Sweeter Configuration II]]
  - [[#elfeed][Elfeed]]
- [[#start-up][Start Up]]
  - [[#encoding][Encoding]]
  - [[#backups][Backups]]
...
* Tricks n Gimmicks for Sweeter Configuration II

Custom stuff that required use-package

** Elfeed

#+BEGIN_SRC emacs-lisp
  (defun shan/elfeed-update-database ()
    "Wrapper to update the elfeed db"
    (interactive)
    (save-current-buffer
      (elfeed)
      (elfeed-db-load)
  ...
  (defun shan/elfeed-search-filters (&optional search-filter-arg)
    ...
    (unless search-filter-arg
      (setq search-filter-arg
            ...
            (read-string "Enter your filter terms:")))
    ...
    (setq search-filter-arg
          (string-join
           ...
           (mapcar (lambda (arg)
                     (cond ((cl-some (lambda (c) (string-prefix-p c arg)) '("-" "@" "#" "+"))
                            arg)
                           ;; determine if term is a time term
                           ((cl-some (lambda (timeword) (string-suffix-p timeword arg)) timewords)
                            (concat "@" arg))
                           ;; heuristic that everything not explicitly excluded is included
                           (t (concat "+" arg))))
                   (split-string search-filter-arg split-string-default-separators))
           " "))
...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package dashboard
      ...
      (use-package dashboard-elfeed
        :if (file-exists-p "~/emacs-dashboard-elfeed/dashboard-elfeed.el")
        :load-path "~/emacs-dashboard-elfeed"
        :custom
        (de/key "b")
        (de/dashboard-search-filter "+test")
